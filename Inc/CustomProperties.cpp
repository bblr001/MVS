...
#include "Resource.h"
#include <atlconv.h>
#include "..\AOIEdit3\PropertiesWnd.h"
#ifdef _DEBUG
#undef THIS_FILE
...
BEGIN_MESSAGE_MAP(CPropSliderCtrl, CSliderCtrl)
	...
	//}}AFX_MSG_MAP
	ON_NOTIFY_REFLECT(NM_RELEASEDCAPTURE, &CPropSliderCtrl::OnNMReleasedcapture)
	ON_WM_KEYUP()
	ON_WM_LBUTTONUP()
	ON_WM_KEYDOWN()
END_MESSAGE_MAP()
...
void CPropSliderCtrl::OnNMReleasedcapture(NMHDR *pNMHDR, LRESULT *pResult)
{
	m_pProp->SetEndEditFlag(true);
	m_pProp->UpdateValue();
	m_pProp->Redraw();
	m_bProcessed = true;
	*pResult = 0;
...
void CPropSliderCtrl::HScroll(UINT/*nSBCode*/, UINT/*nPos*/)
{
	...
	if(m_bKeyHeld == true && m_bKeyReleased == true)
	{
		m_pProp->SetEndEditFlag(true);
...
void CPropSliderCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	m_bKeyReleased = true;
	if(m_bKeyHeld == false)
	{
...
void CPropSliderCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(0x4000 & nFlags) //  the key is pressed
	{
		m_bKeyHeld = true;
	}
	else
...
void CPropSliderCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	m_pProp->SetEndEditFlag(true);
...
CWnd* CSliderProp::CreateInPlaceEdit(CRect rectEdit, BOOL& bDefaultFormat)
{
	...
	if (IFLOAT==m_type)
	{
		...
		pWndSlider->SetPos((int)m_varValue.fltVal*10);
...
BOOL CSliderProp::OnUpdateValue()
{
	...
	if (IFLOAT==m_type)
	{
		...
		m_varValue.fltVal = (float) pSlider->GetPos ()/10.0f;
...
CTwoButtonsProp::CTwoButtonsProp(CString& strName, CString& strValue,CString& helpDescription,CPropertiesWnd* wnd):CMFCPropertyGridProperty(strName, strValue,helpDescription)
...
CCreateButtonsProp::CCreateButtonsProp(CString& strName, CString& strValue,CString& helpDescription,CPropertiesWnd* wnd):CMFCPropertyGridProperty(strName, strValue,helpDescription)
...
CLoadButtonsProp::CLoadButtonsProp(CString& strName, CString& strValue,CString& helpDescription,CPropertiesWnd* wnd):CMFCPropertyGridProperty(strName, strValue,helpDescription)
{
	CBitmap bmp;

...
C2DLineGraph::C2DLineGraph(CPoint g_position, CSize g_size)
{
	...
	m_LegendFont->CreateFont( int(m_Size.cy*0.075), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
		DEFAULT_CHARSET, OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_DONTCARE, _T("Times New Roman") );
...
void C2DLineGraph::BuildGraph(HDC hDC)
{
	...
	int seg_area = int(m_Size.cx*0.90);
	...
	int startX = m_Position.x + int( m_Size.cx*0.05 );//图在区域的X值 ----sam
	int startY = m_Position.y + int( m_Size.cy*0.98);//图在区域的Y值 ---sam
					...
					while ( curr_ser != NULL )
					{
						...
						if ( curr_seg->index > m_DrawSegmentsCompleted )
						{
							...
							while ( next_val != NULL )
							{
								...
								dist = (double(seg_perc)/double(m_SegmentsPercent))*sqrt(pow((double)(val_start.x-val_end.x),2)+pow((double)(val_start.y-val_end.y),2));
...
void C2DLineGraph::SetGraphSize(CSize g_size)
{
	...
	delete m_ValueFont;
	m_ValueFont = new CFont();
	m_ValueFont->CreateFont( int(m_Size.cy*0.15), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
		DEFAULT_CHARSET, OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_DONTCARE, _T("Arial") );
}

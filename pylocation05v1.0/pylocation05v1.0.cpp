// pylocation05v1.0.cpp : 定义 DLL 应用程序的导出函数。
//

#include "stdafx.h"
#include "pylocation05v1.0.h"
...
double  CDoProcess::CalculateEndColumn()
{
	Hobject tmpRect;
	gen_rectangle1(&tmpRect, 900, 3430 - 256, 902, 3430 + 256);
...
double  CDoProcess::CalculateEndRowLeft(double& estimateColLeft, Hlong hDispWnd)
{
	Hobject tmpRect;
	gen_rectangle1(&tmpRect, 420,  4, 550, 8);
	//disp_obj(tmpRect, hDispWnd);
	Hobject reducedRgn;
	reduce_domain(m_currentImage, tmpRect, &reducedRgn);
	...
	HTuple row1,col1,row2,col2;
	smallest_rectangle1(rgn,&row1,&col1, &row2, &col2);
	estimateColLeft = col1[0].D();

	return row1[0].D();
}

double  CDoProcess::CalculateEndRowRight(double& estimateColRight, Hlong hDispWnd)
{
	Hobject tmpRect;
	gen_rectangle1(&tmpRect, 420, m_endCol - 36, 650, m_endCol - 32);
	//disp_obj(tmpRect, hDispWnd);
	...
	Hobject rgn;
	threshold(reducedRgn, &rgn, Mean - 10, 255);
	...
	HTuple row1,col1,row2,col2;
	smallest_rectangle1(rgn,&row1,&col1, &row2, &col2);
	estimateColRight = col1[0].D();
	double ret = row1[0].D();

	return ret;
}

double CDoProcess::CalculateLastFrameRowLeft(double& estimateColLeft, Hlong hDispWnd)
{
	Hobject tmpRect;
	gen_rectangle1(&tmpRect, m_imgHeight-500, 0, m_imgHeight-200, 10);
	//disp_obj(tmpRect, hDispWnd);
	Hobject reducedRgn;
	reduce_domain(m_currentImage, tmpRect, &reducedRgn);
	...
	estimateColLeft = col2[0].D();

	return row2[0].D();
}

double CDoProcess::CalculateLastFrameRowRight(double& estimateColRight, Hlong hDispWnd)
{
	
	Hobject tmpRect;
	gen_rectangle1(&tmpRect, m_imgHeight-500, 3430-60, m_imgHeight-200, 3430-10);
	//disp_obj(tmpRect, hDispWnd);
	Hobject reducedRgn;
	reduce_domain(m_currentImage, tmpRect, &reducedRgn);
...
static bool WeldingLineSelect(Hobject& src, Hobject& dst)
{
	HTuple Area1, Row1, Column1, PointOrder;
	area_center_xld (src, &Area1, &Row1, &Column1, &PointOrder);
	...
	Hobject weldLines;
	select_shape_xld(src, &weldLines, "area", "and", HTuple(Area1[HTuple(Indices[len-2])])-2, 99999);
...
bool  CDoProcess::LocateWeldingLine(int nWeldColHint,WeldRegion& rgnPnts, Hlong hDispWnd)
{
	...
	if(bFirstFrame)
	{
		double estimateColLeft  = 0.0;
		double estimateColRight = 0.0;
		m_dBeginRowLeft  = CalculateEndRowLeft(estimateColLeft, hDispWnd);
		m_dBeginRowRight = CalculateEndRowRight(estimateColRight, hDispWnd);
		double dSlope =  (m_dBeginRowRight - m_dBeginRowLeft) / (estimateColRight - estimateColLeft);
		double dRowHint = m_dBeginRowLeft + dSlope * (nWeldColHint - estimateColLeft);
	...
	if(bLastFrame)
	{ 
		double estimateLeft = 0.0;
		double estimateRight = 0.0;
		double m_dEndRowLeft  = CalculateLastFrameRowLeft(estimateLeft, hDispWnd);
		double m_dEndRowRight = CalculateLastFrameRowRight(estimateRight, hDispWnd);
		double dSlope   = (m_dEndRowRight - m_dEndRowLeft) / (estimateRight - estimateLeft);
		double dRowHint = m_dEndRowLeft + dSlope * (nWeldColHint - estimateLeft);
	...
	{
		gen_rectangle1(&targerRect, 900 - 256, nWeldColHint-100, 900 + 256, nWeldColHint + 100);
	}
	else
	{
		gen_rectangle1(&targerRect, 900 - 256, nWeldColHint-100, 900 + 256, nWeldColHint + 100);
	}
	//disp_obj(targerRect, hDispWnd);
	...
	//disp_obj(SelectedContours, hDispWnd);
	...
	if(lineCounter > 2)
	{
		if(!WeldingLineSelect(SelectedContours, SelectObj))
			return false;
	}
	else
	{
		copy_obj(SelectedContours, &SelectObj, 1, 2);
	}
	//disp_obj(SelectObj, hDispWnd);

	Hobject  UnionContours;
	union_straight_contours_xld(SelectObj, &UnionContours, 200, 0.5, 50, "noparallel", "maximum");

	Hobject weldLines;
	count_obj(UnionContours,&lineCounter);

	Hobject SelectObjLast;
	if(lineCounter[0].I() > 2)
	{
		if(!WeldingLineSelect(UnionContours, SelectObjLast))
			return false;
	}
...
{	
	...
	catch (...) 
	{
		//if (pData->m_isDebug)
		{
			...
			write_string(pData->m_ShowHWindow,"pylocation05v1.0 SheetIron parameter values error,please re-adjust");
#else
			write_string(pData->m_ShowHWindow,"pylocation05v1.0定位算子参数错误");
#endif
		}
		pData->m_isFail          = true;
		testItem->m_bFailSubTest = true;

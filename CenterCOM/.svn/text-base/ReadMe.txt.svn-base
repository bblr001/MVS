========================================================================
    活动模板库 : CenterCOM 项目概述
========================================================================

应用程序向导已为您创建了此 CenterCOM 项目，作为编写动态链接库 (DLL) 
的起点。
...
CenterCOM.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(CenterCOM.h)
        GUID 定义			(CenterCOM_i.c)
        类型库				(CenterCOM.tlb)
        封送处理代码			(CenterCOM_p.c 和 
                                           dlldata.c）

CenterCOM.h
    此文件包含 CenterCOM.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 CenterCOM.pch 的预编译头 (PCH) 文件和
...
CenterCOMps.vcxproj
    ...
    文件。此进程生成 dlldata.c、CenterCOM_i.c 和 
    CenterCOM_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

CenterCOMps.vcxproj.filters

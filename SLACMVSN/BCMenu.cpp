...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		size_t len = sizeof(wchar_t)*(wcslen(szWideString) + 1);
		m_szMenuText = new wchar_t[len];
		if (m_szMenuText)
			wcscpy_s(m_szMenuText,len, szWideString);
    }
	else
...
void BCMenu::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	else
	{
		if(XPflag)
		{
			if(xp_drawmode==BCMENU_DRAWMODE_XP) 
		...
		{
			if(original_drawmode==BCMENU_DRAWMODE_XP)
				DrawItem_WinXP(lpDIS);
			else 
				DrawItem_Win9xNT2000(lpDIS);
		}	
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		nLen = 0;
		size_t len = wcslen((wchar_t *)pTp)+1;

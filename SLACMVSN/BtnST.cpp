...
/*		HDC		hDC = NULL;
		...
		::FillRect(hDC, &rRect, (HBRUSH)RGB(0, 0, 0));
...
//
DWORD CButtonST::SetBitmaps(int nBitmapDis, COLORREF crTransColorDis, int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
	//此函数做了修改，设置了不可用时的图片
	...
	return SetBitmaps(hBitmapDis, crTransColorDis, hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut);
} // End of SetBitmaps
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapDis, COLORREF crTransColorDis, HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		...
		m_csBitmaps[2].crTransparent = crTransColorDis;
		// Get bitmap size
		nRetValue = ::GetObject(hBitmapDis, sizeof(csBitmapSize), &csBitmapSize);
		if (nRetValue == 0)
		{
			FreeResources();
			return BTNST_INVALIDRESOURCE;
		...
		m_csBitmaps[2].hMask = CreateBitmapMask(hBitmapDis, m_csBitmaps[2].dwWidth, m_csBitmaps[2].dwHeight, crTransColorDis);

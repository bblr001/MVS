...
void CGridListCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	//shs
	CString name;
	name = GetItemText( ht.iItem, 0 );

	if(ht.iItem>=0){
		if(name.Left(2)==_T("W_")) 
			m_CurSelWaferNo=ht.iItem+1;					
		else 
			m_CurSelRingTypeNo=ht.iItem+1;

		//GetParent()->SendMessage( ON_WM_UPDATEINFORESULT, m_CurSelWaferNo, m_CurSelRingTypeNo );
	}
	...
	//int state = GetItemState( ht.iItem, LVIS_SELECTED );
...
BOOL CGridListCtrl::PositionControl( CWnd * pWnd, int iItem, int iSubItem )
{
	...
	if( iSubItem >= nColumnCount || GetColumnWidth(Header_OrderToIndex( pHeader->m_hWnd,iSubItem)) < 5 )// We have to take into account that the header may be reordered
	{
		return 0;
	}
